...
var vertices = []float32{
	// 0.0, 0.5, 1.0, 0, 0, // Vertex 1 (X, Y) : Red
	// 0.5, -0.5, 0, 1.0, 0, // Vertex 2 (X, Y) : Green
	// -0.5, -0.5, 0, 0, 1.0, // Vertex 3 (X, Y) : Blue
...
func main() {
	...
	var ebo gl.GLuint
	gl.GlGenBuffers1(&ebo)
	gl.GlBindBuffer(gl.GL_ELEMENT_ARRAY_BUFFER, ebo)
	gl.GlBufferData_i32(gl.GL_ELEMENT_ARRAY_BUFFER, len(elements), elements, gl.GL_STATIC_DRAW)
	...
	checkGlError()
	gl.GlVertexAttribPointer(gl.GLuint(pos_attr), 2, gl.GL_FLOAT, false, 5, 0)
	checkGlError()
	...
	checkGlError()
	gl.GlVertexAttribPointer(gl.GLuint(color_attr), 3, gl.GL_FLOAT, false, 5, 2)
	checkGlError()
...
main_loop:
	for {
		...
		gl.GlDrawElements(gl.GL_TRIANGLES, len(elements), gl.GL_UNSIGNED_INT, 0)
		checkGlError()
